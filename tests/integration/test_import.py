...
def import_files(tmp_path):
    """Self-contained synthetic import cases; never depend on ignored local .log files.

    These exercise file mechanics only. The slow canonical-data test below still
    requires the original file and its exact hash/counts.
    """
    lines = [
        f'10.0.9.05 - nicole_h [15/Mar/2026:11:00:{second:02d} -0400] "GET /home HTTP/1.1" 200 100'
        for second in range(40)
    ]
    lines[12] = lines[4]
    (tmp_path / "march_anchor.log").write_text("\n".join(lines) + "\n", encoding="utf-8")
    mixed = [
        *lines[:4],
        "this line is garbage",
        *lines[5:7],
        '10.0.9.05 - nicole_h [31/Feb/2026:11:00:08 -0400] "GET /home HTTP/1.1" 200 100',
    ]
    (tmp_path / "mixed_rejects.log").write_text("\n".join(mixed) + "\n", encoding="utf-8")
    return tmp_path
...
def test_small_import_is_idempotent_and_keeps_identical_lines_distinct(db, import_files):
    path = import_files / "march_anchor.log"
    ...
    ids = _q(
        db,
        "select event_id, line_number from event_registry where line_number in (5, 13) order by line_number",
    )
    assert len(ids) == 2 and ids[0]["event_id"] != ids[1]["event_id"]
    assert ids[0]["event_id"] == file_event_id(first.content_sha256, 5)
    rows = _q(
        db,
        "select raw_line from raw_events r join event_registry e using (event_id) where line_number in (5,13)",
    )
...
def test_resume_after_partial_import_adds_no_duplicates(db, import_files):
    path = import_files / "march_anchor.log"
...
def test_rejects_are_explicit_and_leading_zero_ip_survives(db, import_files):
    path = import_files / "mixed_rejects.log"
...
def test_full_supplied_dataset_import_matches_overview(db, dataset_path):
    ...
    status = {
        r["status"]: r["n"] for r in _q(db, "select status, count(*) n from raw_events group by status")
    }
