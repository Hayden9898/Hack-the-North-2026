...
Both services deploy **the same repo and the same `Dockerfile`**; they differ only in start command.
...
2. *New -> GitHub Repo* (same repo) again. Name it `worker`.
   Settings -> *Config-as-code path*: `railway.worker.json`. Do **not** give it a domain.
...
The 17 MB dataset is not in git, and the console deliberately refuses filesystem paths, so import over the
network against the deployed database. Use the **public** connection string (Tiger Cloud's, or Railway's TCP
...
### Uploading a dataset through the API instead

`POST /api/v1/datasets` streams the file to `UPLOAD_DIR` and queues the import for the side-effect worker — which
runs in a *different* container with a *different* filesystem. On Railway that job fails visibly with
`upload path missing`. Either import over the network as above, or run one combined service (API plus
`python -m scripts.serve_worker` beside it) with a volume mounted at `UPLOAD_DIR`. Object storage (S3/R2) would
remove the restriction; it is not implemented.
